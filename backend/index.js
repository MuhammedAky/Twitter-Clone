...
app.post("/api/gettweetpage", (req,res) => {
    ...
    var s = (page - 1) * Number(req.body.tweetPage)
...
app.post("/api/followrunfollow", (req,res) => {
            ...
            if (t.isReply) {
                tweet
                    .findById(t.parent, async (err, parentTweet) => {
                        await parentTweet.replies.splice(parentTweet.replies.indeOf(tweetId), 1)
                        parentTweet.save()
                    })
            }
        })
...
app.post("/api/likeorunlikw", (req,res) => {
    var currentUserId = req.body.currentUserId
    var tweetId = req.body.tweetId
