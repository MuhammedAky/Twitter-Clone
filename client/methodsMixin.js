...
export var methodsMixin = {
    methods: {
        ...
        addOrRemoveFromBookmarks(tweetId, add) {
            this.$store.state.isLoading = true
            http.post("/addremovefrombookmarks", {
                username: localStorage.getItem("userId"),
                tweetId,
                add
            })
        ...
        getCurrentUser() {
            ...
            .then(async (result) => {
                ...
                this.$store.state.newInfos.bannerImage = this.$store.state.currentUser.bannerImage || ""
                this.$store.state.newInfos.profileImage = this.$store.currentUser.profileImage || ""
                this.$store.state.isLoading = false
            })
        },
        followOrUnfollow(follow) {
            ...
            if (follow) {
                ...
                this.$store.data.userForProfile.followers.splice(this.$store.state.userForProfile.followers.indexOf(this.$store.state.currentUser._id), 1)
            }
        },
        toggleEditProfilePopup() {
            this.$store.state.editProfilePopup = !this.$store.state.editProfilePopup
            this.$store.state.newInfos.name = this.$store.state.currentUser.name || ""
            this.$store.state.newInfos.mail = this.$store.currentUser.mail || ""
            this.$store.state.newInfos.bio = this.$store.state.currentUser.bio || ""
            this.$store.state.newInfos.location = this.$store.state.currentUser.location || ""
            this.$store.state.newInfos.website = this.$store.state.currentUser.website || ""
            this.$store.state.newInfos.bannerImage = this.$store.state.currentUser.bannerImage || ""
            this.$store.state.newInfos.profileImage = this.$store.state.currentUser.profileImage
        },
        sampleAction(obj) {
